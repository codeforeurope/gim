 ...
 */
Sapo.Handler.Feature = OpenLayers.Class(OpenLayers.Handler, {
	...
	getFeatureFromEvent: function(evt){
		...
		for(var i = 0; i < features.length; ++i){
			...
			feature = features[i];
			xy = feature.layer.getViewPortPxFromLonLat(new OpenLayers.LonLat(feature.geometry.x, feature.geometry.y));
			...
			if( Math.pow(x-xy.x, 2) <= Math.pow(radius, 2) && Math.pow(y-xy.y, 2) <= Math.pow(radius, 2)){
				if (j == 0) {
					matchFeatures[j++] = {
						feature: feature,
					...
					var zIndex = Number(feature.layer.getZIndex());
					if(zIndex >= matchFeatures[0].zIndex){
						if (zIndex > matchFeatures[0].zIndex) {
							matchFeatures = {};
							j = 0;
						}
		...
		if(j == 1){
			return matchFeatures[0].feature;
		}
		else{
			//TBD: The features are from the same layer, check whcih has been added last
			return matchFeatures[0].feature;
		}
	},
...
//        }
    ...
    CLASS_NAME: "Sapo.Handler.Feature"
});
